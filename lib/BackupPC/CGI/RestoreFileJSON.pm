...
package BackupPC::CGI::RestoreFileJSON;
...
sub restoreFile
{
    ...
    %Conf = $bpc->Conf();
    ServerConnect();
    ErrorJSON($Lang->{Empty_host_name}) if ( $host eq "" );
    ...
    if ( $dir =~ m{(^|/)\.\.(/|$)} || !defined($a) ) {
        $dir = decode_utf8($dir);
        ErrorJSON("Can't restore bad file ${EscHTML($dir)} (${EscHTML($num)}, ${EscHTML($share)})");
    }
    my $f = BackupPC::XS::FileZIO::open($a->{fullPath}, 0, $a->{compress});
    if ( !defined($f) ) {
        my $fullPath = decode_utf8($a->{fullPath});
        ErrorJSON("Unable to open file ${EscHTML($fullPath)} (${EscHTML($num)}, ${EscHTML($share)})");
    }
    my $data;
